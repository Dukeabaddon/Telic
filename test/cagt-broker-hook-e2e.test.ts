...
describe("CAGT broker hook E2E", () => {
  it("denies repository.write for report_only runs with an active session", () => {
    const repositoryRoot = mkdtempSync(join(tmpdir(), "telic-broker-hook-repo-"));
    const stateDirectory = mkdtempSync(join(tmpdir(), "telic-broker-hook-state-"));
  ...
  it("allows repository.write for fix_only runs inside granted scope", () => {
    const repositoryRoot = mkdtempSync(join(tmpdir(), "telic-broker-hook-fix-"));
    const stateDirectory = mkdtempSync(join(tmpdir(), "telic-broker-hook-fix-state-"));
    const previousStateDir = process.env.TELIC_STATE_DIR;
    process.env.TELIC_STATE_DIR = stateDirectory;
    mkdirSync(join(repositoryRoot, "src"), { recursive: true });
